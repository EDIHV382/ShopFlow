<template>
  <div class="space-y-6">
    ...
    <!-- Add form -->
    <div class="card p-5">
      <h3 class="font-semibold text-dark-100 mb-4">{{ editingCat ? 'Editar categoría' : 'Nueva categoría' }}</h3>
      <div class="flex gap-3">
        <input v-model="form.name" type="text" class="input" placeholder="Nombre" @input="autoSlug" />
        <input v-model="form.slug" type="text" class="input" placeholder="slug-url" />
        <button @click="handleSave" :disabled="saving" class="btn btn-primary btn-sm whitespace-nowrap">
    ...
    <div class="card overflow-hidden">
      ...
      <table v-else class="w-full text-sm">
        ...
        <tbody class="divide-y divide-dark-800">
          <tr v-for="cat in categories" :key="cat.id" class="hover:bg-dark-800/50 transition-colors">
            ...
            <td class="px-4 py-3 text-center">
              <div class="flex justify-center gap-2">
                <button @click="startEdit(cat)" class="btn btn-secondary btn-sm px-3">Editar</button>
                <button @click="confirmDelete(cat)" class="btn btn-danger btn-sm px-3">Eliminar</button>
              </div>
            </td>
          </tr>
        </tbody>
...
function confirmDelete(cat: Category) { deletingCat.value = cat }

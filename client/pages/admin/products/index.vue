<template>
  <div class="space-y-6">
    <div class="flex items-center justify-between">
      ...
      <NuxtLink to="/admin/products/create" class="btn btn-primary btn-sm">+ Nuevo producto</NuxtLink>
    </div>
      ...
      <div class="overflow-x-auto">
        <table class="w-full text-sm">
          ...
          <tbody class="divide-y divide-dark-800">
            <tr v-for="product in products" :key="product.id" class="hover:bg-dark-800/50 transition-colors">
              <td class="px-4 py-3">
                <div class="flex items-center gap-3">
                  ...
                  <span class="font-medium text-dark-100 truncate max-w-[200px]">{{ product.name }}</span>
                </div>
              </td>
              <td class="px-4 py-3 text-dark-400">{{ product.category_name || '—' }}</td>
              <td class="px-4 py-3 text-right font-semibold text-white">{{ formatPrice(product.price) }}</td>
              <td class="px-4 py-3 text-right">
                <span :class="product.stock === 0 ? 'badge-sold-out' : product.stock <= 5 ? 'badge bg-yellow-500/20 text-yellow-400 border border-yellow-500/30' : 'badge bg-green-500/20 text-green-400 border border-green-500/30'">
                  {{ product.stock }}
                </span>
              </td>
              <td class="px-4 py-3 text-center">
                <div class="flex items-center justify-center gap-2">
                  <NuxtLink :to="`/admin/products/${product.id}/edit`" class="btn btn-secondary btn-sm px-3">Editar</NuxtLink>
                  <button @click="confirmDelete(product)" class="btn btn-danger btn-sm px-3">Eliminar</button>
                </div>
              </td>
            </tr>
          </tbody>
    ...
    <Pagination v-if="meta" :current-page="page" :total-pages="meta.totalPages" @change="loadProducts" />
...
async function loadProducts(newPage = 1) {
  page.value = newPage
  loading.value = true
  const result = await api.get<PaginatedResponse<Product>>('/products', { page: page.value, limit: 20 })
  products.value = result.data
  meta.value = result.meta
  loading.value = false
}

function confirmDelete(product: Product) { deletingProduct.value = product }

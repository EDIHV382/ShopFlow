  ...
  <div class="min-h-screen flex bg-dark-950">
    ...
    >
      ...
      <div class="flex items-center gap-3 h-16 px-4 border-b border-dark-800">
        <div
          class="w-8 h-8 rounded-xl bg-gradient-to-br from-primary-500 to-accent-500 flex items-center justify-center flex-shrink-0"
        >
          <span class="text-white font-bold text-sm">S</span>
        </div>
        <span
          v-if="sidebarOpen"
          class="font-bold gradient-text text-lg transition-opacity duration-200"
          >ShopFlow</span
        >
      </div>
    ...
    <!-- Main content -->
    <div
      :class="['flex-1 flex flex-col transition-all duration-300', sidebarOpen ? 'ml-64' : 'ml-16']"
    >
      <!-- Top bar -->
      <header
        class="h-16 flex items-center justify-between px-6 bg-dark-900 border-b border-dark-800 sticky top-0 z-40"
      >

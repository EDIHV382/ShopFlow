    ...
    <div class="container-page">
      <div class="flex items-center justify-between h-16">
        ...
        <NuxtLink to="/" class="flex items-center gap-2.5 group">
          <div class="w-8 h-8 rounded-xl bg-gradient-to-br from-primary-500 to-accent-500 flex items-center justify-center shadow-glow group-hover:shadow-glow-accent transition-all duration-300">
        ...
        <nav class="hidden md:flex items-center gap-1">
          <NuxtLink to="/products" class="px-4 py-2 rounded-xl text-sm font-medium text-dark-300 hover:text-dark-100 hover:bg-dark-800 transition-all duration-200">
            Productos
          </NuxtLink>
          <template v-if="authStore.isAdmin">
            <NuxtLink to="/admin/dashboard" class="px-4 py-2 rounded-xl text-sm font-medium text-accent-400 hover:bg-accent-500/10 transition-all duration-200">
              Admin
            </NuxtLink>
          </template>
        </nav>
        ...
        <div class="flex items-center gap-2">
          <!-- Cart button -->
          <NuxtLink to="/cart" class="relative p-2.5 rounded-xl hover:bg-dark-800 text-dark-300 hover:text-dark-100 transition-all duration-200 group">
          ...
          <template v-else>
            ...
            <div class="relative" ref="userMenuRef">
              ...
              >
                <div class="w-7 h-7 rounded-full bg-gradient-to-br from-primary-500 to-accent-500 flex items-center justify-center">
                  <span class="text-white font-bold text-xs">{{ initials }}</span>
                </div>
                <span class="text-sm font-medium text-dark-200 hidden sm:block max-w-20 truncate">{{ authStore.userName }}</span>
                <svg xmlns="http://www.w3.org/2000/svg" class="w-4 h-4 text-dark-400" fill="none" viewBox="0 0 24 24" stroke="currentColor">
              ...
              <Transition name="dropdown">
                ...
                >
                  <NuxtLink to="/orders" @click="userMenuOpen = false"
                  ...
                  <template v-if="authStore.isAdmin">
                    <NuxtLink to="/admin/dashboard" @click="userMenuOpen = false"

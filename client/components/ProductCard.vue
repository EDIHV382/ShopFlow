<template>
  ...
  >
    ...
    <div class="relative aspect-square overflow-hidden rounded-t-2xl bg-dark-800">
      ...
      <!-- Sold out overlay -->
      <div v-if="product.stock === 0" class="absolute inset-0 bg-dark-950/70 flex items-center justify-center">
    ...
    <!-- Content -->
    <div class="p-4">
      <h3 class="font-semibold text-dark-100 text-sm leading-tight mb-1 line-clamp-2 group-hover:text-primary-400 transition-colors">
        {{ product.name }}
      </h3>
      ...
      <div class="flex items-center justify-between">
        ...
        <span :class="product.stock > 0 ? 'text-green-400' : 'text-red-400'" class="text-xs font-medium">
      ...
      <button v-else disabled class="mt-3 w-full btn btn-secondary btn-sm opacity-50 cursor-not-allowed">
        Sin stock
      </button>
    </div>
  </article>

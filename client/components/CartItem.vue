<template>
  <div class="flex gap-4 p-4 card">
    ...
    <div class="flex-1 min-w-0">
      <h4 class="text-sm font-semibold text-dark-100 leading-tight truncate">{{ item.product_name }}</h4>
      ...
      <div class="flex items-center gap-2 mt-3">
        ...
        >−</button>
        <span class="text-sm font-semibold text-dark-100 min-w-[24px] text-center">{{ item.quantity }}</span>
        <button
          @click="increase"
    ...
    <div class="flex flex-col items-end justify-between">
      <button @click="handleRemove" class="text-dark-500 hover:text-red-400 transition-colors p-1" title="Eliminar">
        ✕
      </button>
      <span class="text-base font-bold text-white">{{ formatPrice(item.product_price * item.quantity) }}</span>
    </div>
  </div>
</template>
...
interface Props { item: CartItemLocal }

...
export function applyRateLimiter(req: VercelRequest, res: VercelResponse, limiter: ReturnType<typeof rateLimit>): void {
  let key = 'anonymous';
...
export function applyMiddleware(req: VercelRequest, res: VercelResponse, isAuthRoute = false): void {
...
export function requireAdmin(req: VercelRequest, res: VercelResponse): JwtPayload | null {
  ...
  if (req.method !== 'GET') {
    res.status(403).json({
      error: '🔒 Modo de Prueba: Las modificaciones en la base de datos están deshabilitadas en esta demostración.',
    });
    return null;
  }

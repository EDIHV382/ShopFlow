...
import { z } from 'zod';
import type { CartItem } from '../_lib/types.js';
...
export default async function handler(req: VercelRequest, res: VercelResponse) {
  ...
  if (item.user_id !== payload.userId) return res.status(403).json({ error: 'No autorizado' });
  ...
  if (req.method === 'PUT') {
    ...
    // Check stock
    const product = await queryOne<{ stock: number }>('SELECT stock FROM products WHERE id = $1', [item.product_id]);
